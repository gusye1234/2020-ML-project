...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = pytrace.from_numpy(np.vstack([e.state for e in experiences if e is not None])).float()
        actions = pytrace.from_numpy(np.vstack([e.action for e in experiences if e is not None])).long()
        rewards = pytrace.from_numpy(np.vstack([e.reward for e in experiences if e is not None])).float()
        next_states = pytrace.from_numpy(np.vstack([e.next_state for e in experiences if e is not None])).float()
        dones = pytrace.from_numpy(np.vstack([e.done for e in experiences if e is not None]).astype(np.uint8)).float()
